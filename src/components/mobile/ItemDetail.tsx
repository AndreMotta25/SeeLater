...
export function ItemDetail({ item, onMarkAsViewed, onDelete, onMarkAsUnviewed }: ItemDetailProps) {
      ...
      <div className="max-w-lg mx-auto px-4 -mt-6 relative z-10">
        ...
        <div className="flex flex-wrap items-center gap-2 mb-2">
          ...
          <span className="px-3 py-1 rounded-full bg-[#2A2A3E] text-white text-xs font-semibold">
            {item.type === 'video' ? 'Vídeo' : item.type === 'article' ? 'Artigo' : item.type === 'repo' ? 'Repositório' : item.type === 'tweet' ? 'Tweet' : 'Outro'}
          </span>
          {item.category && (
            <span className="px-3 py-1 rounded-full bg-[#2A2A3E] text-white text-xs font-semibold">
              {item.category}
        ...
        <div className="space-y-3 mb-6">
          ...
          >
            <svg width="20" height="20" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
              ...
              <path d="M15 3H21V9M21 3L14 10" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>
            </svg>
            Abrir link
          </a>
